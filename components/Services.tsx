          ...
          {SERVICES.map((service, index) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-3">
                 ...
                 <span className="inline-flex items-center justify-center px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 text-[10px] font-bold" aria-label={`${service.features.length} core features listed`}>
        ...
        <div className={`mt-24 transition-all duration-1000 delay-300 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="text-center mb-12">
             <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-xs font-bold uppercase tracking-wider mb-4">
                <Layers size={14} aria-hidden="true" /> Technology Stack
             </div>
             ...
             <div className="flex overflow-x-auto no-scrollbar border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 p-2 gap-2" role="tablist">
                {TECH_DOMAINS.map((domain) => (
                  <button
                    ...
                    className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold transition-all duration-300 whitespace-nowrap active:scale-95 ${
                      activeTab === domain.id
                ...
                {TECH_DOMAINS.map((domain) => (
                  ...
                  >
                    {domain.skills.map((skill, idx) => (
                      <div 
                        key={skill} 
                        className="flex items-center gap-3 p-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-700 hover:border-blue-200 dark:hover:border-blue-800 transition-all duration-300 group cursor-default hover:bg-white dark:hover:bg-slate-800 hover:shadow-md"
                      >
                         <div className="w-8 h-8 rounded-lg bg-white dark:bg-slate-800 flex items-center justify-center text-slate-400 group-hover:text-blue-500 transition-colors shadow-sm border border-slate-100 dark:border-slate-700" aria-hidden="true">
                            <Terminal size={16} />
                         </div>
                    ...
                    <div className="col-span-1 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl flex items-center justify-center p-4 text-slate-400 text-sm font-medium hover:border-blue-300 dark:hover:border-blue-700 transition-all cursor-default">
                       And more...
                    </div>
                  </div>
                ))}
